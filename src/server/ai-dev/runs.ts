...
import { appendAiDevEvent } from './events'
import {
  toPublicRepo,
  toPublicRun,
  type AiDevRepoRow,
...
export async function applyCursorWebhook(input: {
  deliveryId: string
  agentId: string
  status: string
  ...
  const { data, error } = await supabase
    .from('ai_dev_runs')
    .select('*')
    .eq('cursor_agent_id', input.agentId)
    .maybeSingle()
  if (error) throw new Error(error.message)
  if (!data) return { ok: true, ignored: 'no_matching_run' as const }

  const run = data as AiDevRunRow
  if (
  ...
  const { data: repo } = await supabase
    ...
    .eq('id', run.ai_dev_repo_id)
    .maybeSingle()
  if (!repo || (repo as AiDevRepoRow).team_id !== run.team_id) {
    return { ok: true, ignored: 'repo_mismatch' as const }
  }
  ...
  return { ok: true, run: toPublicRun(await reconcileRun({ ...run, last_reconciled_at: null })) }
}

export async function applyGitHubAppWebhook(input: {
  deliveryId: string
  eventName: string
  ...
  const repoRows = ((repos ?? []) as AiDevRepoRow[]).filter(
    (row) =>
  ...
  const matched = ((runs ?? []) as AiDevRunRow[]).filter((run) => {
  ...
  for (const run of matched) {
    updated.push(await reconcileRun({ ...run, last_reconciled_at: null }))
  }
